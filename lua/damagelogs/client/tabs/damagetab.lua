...
function Damagelog:DrawDamageTab(x, y)
    local function askLogs()
        if not self.SelectedRound then
            return
        end
    ...
    self.Highlight.DoClick = function(self)
        ...
        if table.HasValue(Damagelog.Highlighted, selected) then
            return
        end
...
net.Receive("DL_SendDamagelog", function()
    ...
    if not IsValid(Damagelog.Menu) then
        return
    end
...
net.Receive("DL_RefreshDamagelog", function()
    ...
    if not IsValid(client) then
        return
    end -- sometimes happens while joining
